...
```json
{
  ...
  "generated_prompts": {
    "hero_motif_1": "英文透明背景服装定位印花图案生成提示词，约100-300词。基于上方 struct 结构化数据中 subject.primary（主主体）和 subject.secondary（次要主体）的完整描述，提炼并扩展成一个可直接提交给 AI 图像模型的最终英文 prompt，无需任何后处理。此图案用于商业成衣（T恤、衬衫、防晒服等上装）的定位印花（placement print），印在前胸、后背或侧缝等服装部位，不是装饰画、海报或贴纸。必须逐条包含：1) 主体精确类别与数量；2) 整体轮廓与体量感；3) 主色/辅色/点缀色（附近似 HEX 值）；4) 材质纹理与表面光学特性（哑光/镜面/透明/发光）；5) 姿态、动作、朝向；6) 所有关键细节（花纹、配饰、表情、肌理、边缘特征）；7) 与背景完全分离的明确指令。强制关键词（必须原样包含）：isolated foreground subject only, transparent PNG cutout, real alpha background, no background, no scenery, no ground plane, no text, no logo, no watermark, centered complete subject, full uncropped figure, generous transparent margin around subject, soft clean anti-aliased edges, apparel placement graphic, commercial garment print。严禁：服装版型、褶皱、人体穿着效果、模特、光影场景、背景画面、画框/贴纸排版、渐变色背景、纯色背景盒、3D 渲染效果。",
